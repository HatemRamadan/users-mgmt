...
import com.sumerge.program.exception.NoSuchGroup;
...
public class TestingClassIT {
    ...
    public void test04addGroup(){
        ...
        List<Group> group2 = client.target("http://localhost:8880/groups/").register(adminCred).register(JacksonJsonProvider.class)
                .request(MediaType.APPLICATION_JSON)
                .get().readEntity(new GenericType<List<Group>>() {});
        groupID=group2.get(group2.size()-1).getGroupid();
        Assert.assertTrue(group2.get(group2.size()-1).getName().equals(group.getName()));
    }

    @Test(expected = InternalServerErrorException.class)
    public void test04deleteGroup() {
        Client client = ClientBuilder.newClient();
        Response response = client.target("http://localhost:8880/group/"+groupID).register(adminCred).register(JacksonJsonProvider.class)
                .request(MediaType.APPLICATION_JSON)
                .delete();
        Group group2 = client.target("http://localhost:8880/group/"+groupID).register(adminCred).register(JacksonJsonProvider.class)
                .request(MediaType.APPLICATION_JSON)
                .get(Group.class);
    }
}